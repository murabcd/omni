...
export type BotEnvConfig = {
	...
	OBSERVATIONS_TRIGGER: number;
	OBSERVATIONS_TAIL: number;
	OBSERVATIONS_MAX_MESSAGES: number;
	OBSERVATIONS_MAX_CHARS: number;
	OBSERVATIONS_ASYNC: boolean;
	...
	TELEGRAM_ABORT_ON_NEW_MESSAGE: boolean;
	TELEGRAM_REACTION_NOTIFICATIONS: string;
	TELEGRAM_REACTION_LEVEL: string;
	...
	TASK_AUTO_URL_THRESHOLD: number;
	TASK_AUTO_MIN_CHARS: number;
	TASK_AUTO_KEYWORDS: string;
	TASK_PROGRESS_MIN_MS: number;
	...
	WORKER_MEDIA_TIMEOUT_MS: number;
	TOOL_SERVICE_URL: string;
	TOOL_SERVICE_SECRET: string;
	TOOL_SERVICE_TIMEOUT_MS: number;
	...
	INBOUND_DEDUPE_TTL_MS: number;
	INBOUND_DEDUPE_MAX: number;
	SERVICE_NAME: string;
...
export function loadBotEnv(env: BotEnv): BotEnvConfig {
	...
	const OBSERVATIONS_TRIGGER = Number.parseInt(
		env.OBSERVATIONS_TRIGGER ?? env.HISTORY_SUMMARY_TRIGGER ?? "60",
		10,
	);
	const OBSERVATIONS_TAIL = Number.parseInt(
		env.OBSERVATIONS_TAIL ?? env.HISTORY_SUMMARY_TAIL ?? "20",
		10,
	);
	const OBSERVATIONS_MAX_MESSAGES = Number.parseInt(
		env.OBSERVATIONS_MAX_MESSAGES ?? env.HISTORY_SUMMARY_MAX ?? "200",
		10,
	);
	const OBSERVATIONS_MAX_CHARS = Number.parseInt(
		env.OBSERVATIONS_MAX_CHARS ?? env.HISTORY_SUMMARY_MAX_CHARS ?? "12000",
		10,
	);
	const OBSERVATIONS_ASYNC = env.OBSERVATIONS_ASYNC === "1";
	...
	const TELEGRAM_LINK_PREVIEW = env.TELEGRAM_LINK_PREVIEW !== "0";
	const TELEGRAM_ABORT_ON_NEW_MESSAGE =
		env.TELEGRAM_ABORT_ON_NEW_MESSAGE === "1";
	const TELEGRAM_REACTION_NOTIFICATIONS =
		env.TELEGRAM_REACTION_NOTIFICATIONS ?? "off";
	const TELEGRAM_REACTION_LEVEL = env.TELEGRAM_REACTION_LEVEL ?? "off";
	...
	const TASKS_ENABLED = env.TASKS_ENABLED !== "0";
	const TASK_AUTO_URL_THRESHOLD = Number.parseInt(
		env.TASK_AUTO_URL_THRESHOLD ?? "3",
		10,
	);
	const TASK_AUTO_MIN_CHARS = Number.parseInt(
		env.TASK_AUTO_MIN_CHARS ?? "800",
		10,
	);
	const TASK_AUTO_KEYWORDS = env.TASK_AUTO_KEYWORDS ?? "";
	const TASK_PROGRESS_MIN_MS = Number.parseInt(
		env.TASK_PROGRESS_MIN_MS ?? "5000",
		10,
	);
	...
	const WORKER_MEDIA_URL = env.WORKER_MEDIA_URL ?? "";
	const WORKER_MEDIA_TIMEOUT_MS = Number.parseInt(
		env.WORKER_MEDIA_TIMEOUT_MS ?? "20000",
		10,
	);
	const TOOL_SERVICE_URL = env.TOOL_SERVICE_URL ?? "";
	const TOOL_SERVICE_SECRET = env.TOOL_SERVICE_SECRET ?? "";
	const TOOL_SERVICE_TIMEOUT_MS = Number.parseInt(
		env.TOOL_SERVICE_TIMEOUT_MS ?? "20000",
		10,
	);
	...
	const INBOUND_DEDUPE_TTL_MS = Number.parseInt(
		env.INBOUND_DEDUPE_TTL_MS ?? "1200000",
		10,
	);
	const INBOUND_DEDUPE_MAX = Number.parseInt(
		env.INBOUND_DEDUPE_MAX ?? "5000",
		10,
	);
